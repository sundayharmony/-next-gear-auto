...
export function CustomerDetailDrawer({
  ...
  const loadCustomerData = async (target: CustomerRow) => {
    ...
    try {
      ...
      try {
        const ticketsRes = await adminFetch(`/api/admin/tickets?customer_id=${encodeURIComponent(target.id)}`);
        if (ticketsRes.ok) {
  ...
  const saveCustomerEdit = async () => {
    if (!customer || !editName || !editEmail) {
  ...
  const handleCustomerDocUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (!customer || !e.target.files?.[0]) return;
    ...
    try {
      ...
      if (data.success || data.url) {
        ...
        setCustomer((prev) => prev ? { ...prev, idDocumentUrl: data.url } : prev);
  ...
  const removeProfilePicture = async () => {
    ...
    try {
      ...
      setCustomers((prev) =>
        prev.map((c) =>
          c.id === customer.id ? { ...c, profilePictureUrl: undefined } : c
        )
      );
  ...
  const saveProfilePicture = async () => {
    if (!cropPreview || !customer) return;
    ...
    try {
      ...
      const formData = new FormData();
      formData.append("file", blob, `profile_${customer.id}.jpg`);
      formData.append("customerId", customer.id);
      ...
      if (data.success) {
        ...
        setCustomer((prev) => (prev ? { ...prev, profilePictureUrl: data.url } : prev));
        // Also update the customer in the list so the avatar updates everywhere
        setCustomers((prev) =>
          prev.map((c) =>
            c.id === customer.id ? { ...c, profilePictureUrl: data.url } : c
          )
        );
        if (data.warning) {
  ...
  useEffect(() => {
    if (customer?.profilePictureUrl) {
      setProfilePictureUrl(customer.profilePictureUrl);
    } else {
      setProfilePictureUrl(null);
    }
  }, [customer]);
  ...
  const latestIdUrl = customer?.idDocumentUrl || customerBookings.find((b) => b.id_document_url)?.id_document_url;
                    ...
                    <CardContent className="p-5">
                      ...
                      <div className="space-y-3">
                        ...
                        <div>
                          ...
                          <p className="text-lg font-bold text-black">{formatDate(customer.createdAt)}</p>
                        </div>
                        {stats?.firstBooking && (
                          <div>
