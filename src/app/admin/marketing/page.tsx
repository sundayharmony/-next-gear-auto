...
export default function AdminMarketingPage() {
  ...
  const sendCampaign = async () => {
    ...
    try {
      const res = await adminFetch("/api/admin/marketing/send-campaign", {
        ...
        timeoutMs: 300000, // 5 minute timeout for campaign sends
      });
  ...
  return (
    <>
      ...
      <AdminPageBody className="space-y-4 sm:space-y-6">
        ...
        <div className="grid gap-4 sm:gap-6 lg:grid-cols-2">
          <Card>
            <CardContent className="p-3 sm:p-5 space-y-3 sm:space-y-4">
              <h2 className="text-base sm:text-lg font-semibold text-gray-900 dark:text-gray-100">Email</h2>
              <div>
                <label htmlFor="campaign-subject" className="block text-xs sm:text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                  Subject
                </label>
                <Input
                  id="campaign-subject"
          ...
          <Card>
            <CardContent className="p-3 sm:p-5 space-y-3 sm:space-y-4">
              <div className="flex items-center justify-between gap-2">
                <h2 className="text-base sm:text-lg font-semibold text-gray-900 dark:text-gray-100 flex items-center gap-1.5 sm:gap-2">
                  <Car className="h-4 w-4 sm:h-5 sm:w-5 text-purple-600" />
                  Vehicles
                </h2>
                <div className="flex gap-1.5 sm:gap-2">
                  <Button type="button" variant="outline" size="sm" onClick={selectAllVehicles} className="text-xs sm:text-sm px-2 sm:px-3">
                    All
                  </Button>
                  <Button type="button" variant="ghost" size="sm" onClick={clearVehicles} className="text-xs sm:text-sm px-2 sm:px-3">
                    Clear
                  </Button>
                </div>
              </div>
              <Button type="button" variant="secondary" size="sm" onClick={insertVehiclePlaceholder} className="w-full text-xs sm:text-sm">
              ...
              {vehiclesLoading ? (
                ...
                <p className="text-xs sm:text-sm text-gray-500">No published vehicles found.</p>
              ) : (
                <div className="grid gap-2 sm:gap-3 max-h-[280px] sm:max-h-[420px] overflow-y-auto pr-1 sm:grid-cols-2">
                  {publishedVehicles.map((v) => {
                    ...
                    return (
                      <button
                        ...
                        className={`text-left rounded-lg border p-1.5 sm:p-2 transition-colors ${
                          selected
                      ...
                      >
                        <div className="flex gap-1.5 sm:gap-2">
                          <div className="relative h-11 w-16 sm:h-14 sm:w-20 shrink-0 overflow-hidden rounded-md bg-gray-100">
                            {thumb ? (
                              <Image
                                src={thumb}
                                alt=""
                              ...
                              <div className="flex h-full items-center justify-center text-[10px] sm:text-xs text-gray-400">
                                No img
                              </div>
                            )}
                          </div>
                          <div className="min-w-0 flex-1">
                            <p className="font-medium text-xs sm:text-sm truncate">{name}</p>
                            <p className="text-xs sm:text-sm text-purple-700 font-semibold">${v.dailyRate}/day</p>
                            <Badge
                              className={
                                v.isAvailable
                                  ? "bg-green-100 text-green-800 text-[9px] sm:text-[10px] mt-0.5 sm:mt-1"
                                  : "bg-amber-100 text-amber-800 text-[9px] sm:text-[10px] mt-0.5 sm:mt-1"
                              }
                            >
                          ...
                          <span className="shrink-0 text-purple-600">
                            {selected ? (
                              ...
                              <Square className="h-4 w-4 sm:h-5 sm:w-5 text-gray-300" />
                            )}
                          </span>
                        </div>
                      </button>
          ...
          <CardContent className="p-3 sm:p-5 space-y-3 sm:space-y-4">
            ...
            {recipientMode === "selected" && (
              <div className="space-y-2 sm:space-y-3">
                ...
                {customersLoading ? (
                  ...
                  <div className="max-h-48 sm:max-h-64 overflow-y-auto rounded-lg border border-gray-200 dark:border-gray-700 divide-y divide-gray-100 dark:divide-gray-800">
                    {customers.length === 0 ? (
                      <p className="p-3 sm:p-4 text-xs sm:text-sm text-gray-500">No customers match your search.</p>
                    ) : (
                      customers.map((c) => {
                        ...
                        return (
                          <label
                            key={c.id}
                            className="flex items-center gap-2 sm:gap-3 px-3 py-2.5 sm:px-4 sm:py-3 cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-800/50"
                          >
                            <input
                              type="checkbox"
                              checked={checked}
                              onChange={() => toggleCustomer(c.id)}
                              className="rounded border-gray-300 text-purple-600 focus:ring-purple-500 h-3.5 w-3.5 sm:h-4 sm:w-4"
                            />
                            <span className="min-w-0 flex-1">
                              <span className="block text-xs sm:text-sm font-medium truncate">{c.name || "—"}</span>
                              <span className="block text-[11px] sm:text-xs text-gray-500 truncate">{c.email}</span>
                            </span>
                          </label>
                        );
                      })
            ...
            {recipientMode === "import" && (
              <div className="space-y-1.5 sm:space-y-2">
                ...
                <textarea
                  className="w-full min-h-[100px] sm:min-h-[120px] rounded-lg border border-gray-200 px-2.5 py-2 sm:px-3 text-xs sm:text-sm dark:border-gray-700 dark:bg-gray-900"
                  value={importText}
                  onChange={(e) => setImportText(e.target.value)}
                  placeholder="customer@example.com&#10;another@example.com"
                />
                <p className="text-[11px] sm:text-xs text-gray-500">{importEmails.length} address(es) detected</p>
              </div>
            )}
            ...
            <div className="pt-2 border-t border-gray-100 dark:border-gray-800 flex flex-col sm:flex-row sm:flex-wrap items-start sm:items-center justify-between gap-2 sm:gap-3">
              <p className="text-xs sm:text-sm text-gray-600 dark:text-gray-400">
