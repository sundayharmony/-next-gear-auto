    ...
    <>
      {showMore ? (
        <>
          ...
          <div
            className="fixed bottom-0 left-0 right-0 z-[90] animate-in slide-in-from-bottom-2"
            role="dialog"
            aria-modal="true"
            aria-labelledby={moreTitleId}
            ref={sheetRef}
          >
            <div className="mx-2 mb-[calc(env(safe-area-inset-bottom,0px)+4.75rem)] flex max-h-[min(72dvh,calc(100dvh-8rem))] flex-col overflow-hidden rounded-2xl border border-gray-200/50 bg-white/95 shadow-2xl backdrop-blur-xl">
              <div className="flex shrink-0 items-center justify-between px-4 py-2.5 border-b border-gray-100">
                ...
                <button
                  type="button"
                  onClick={() => setShowMore(false)}
                  className="inline-flex h-11 w-11 items-center justify-center rounded-full bg-gray-100 hover:bg-gray-200 active:bg-gray-300 transition-colors"
              ...
              <div className="flex shrink-0 items-center justify-between border-b border-gray-100 px-4 py-3">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-medium text-gray-800">{isDark ? "Dark Mode" : "Light Mode"}</span>
                </div>
                <button
                  type="button"
                  onClick={toggleTheme}
              ...
              <div className="nga-overlay-scroll min-h-0 flex-1 overflow-y-auto overscroll-contain">
                <div className={cn("grid gap-1 p-2", moreGridCols === 2 ? "grid-cols-2" : "grid-cols-3")}>
                  {moreItems.map((item) => {
                    ...
                    return (
                      <Link
                        ...
                        className={cn(
                          "relative flex min-h-[72px] flex-col items-center justify-center gap-1 py-3 px-2 rounded-xl transition-all active:scale-95",
                      ...
                      >
                        ...
                        {item.badge != null && item.badge > 0 && (
                          <span className="absolute right-3 top-2 inline-flex h-4 min-w-4 items-center justify-center rounded-full bg-purple-600 px-1 text-[9px] font-bold text-white">
                            {item.badge}
                          </span>
                        )}
      ...
      <nav className="fixed inset-x-0 bottom-0 shrink-0 z-[91] lg:hidden" aria-label={ariaLabel}>
        <div className="border-t border-gray-200/50 bg-white/85 shadow-[0_-2px_20px_rgba(0,0,0,0.06)] backdrop-blur-xl">
          <div className="flex items-stretch justify-around px-1 pb-[calc(env(safe-area-inset-bottom,0px)+2px)]">
            {primaryTabs.map((tab) => {
              ...
              return (
                <Link
                  ...
                  aria-current={active ? "page" : undefined}
                  className={cn(
                    "flex min-h-14 flex-col items-center justify-center gap-0.5 pt-1.5 pb-1 px-2 min-w-[56px] transition-all active:scale-90",
